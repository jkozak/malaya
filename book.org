...
It is a language and runtime for the concise and rapid expression,
implementation and deployment of rule-based persistent reactive
systems.
...
appropriate data out.  They typically run without stopping and are
usually called daemons or services.  Malaya is a micro-kernel for this
class of software.
...
Malaya is designed to facilitate building such systems.  All data is
JSON; [[prevalence][persistent storage]] is based around a hash-chained journal of
updates which is tamper-proof and maintains the system's entire
history; it is thus eminently auditable.  It is also intended to be
very amenable to formal methods.
...
It consists of:
 ...
 - [[plugins][plugins]] currently available implement
  - timers
  - tcp and udp sockets
  - http, https
  - websockets
...
It is available under the [[https://www.gnu.org/licenses/agpl-3.0.en.html][AGPL]] licence.

The initial reference implementation is layered on top of the familiar
[[http://nodejs.org/][nodejs]] framework.
...
module.exports.main = store {
    ...
    rule (-['query',   {id,flags:{opcode,...}, q,...},    {src:['udp',...addr]}],
          ...
          +['response',{id,flags:{opcode,rc:0},q,
                        ans:rds.map(rd=>{rd}) },          {dst:['udp',...addr]} ] );
...
Whitespace is not significant, but is used here to show grouping by
alignment.
...
*** =file=
*** =hash=
Provides access to Malaya's internal hash store for stashing [[http://wikipedia.org/wiki/Binary Large Object][BLOB]]s and so on.
*** =http=
*** =tcp=
*** =process=
*** =random=
...
Provides information about hardware attached to the system, including
updates (e.g. USB devices attached or removed).
